...
def load_proxies(file_path='proxies.txt'):
    if os.path.exists(file_path):
        ...
        if not proxies:
            print(f"{get_wib_timestamp()} | {Fore.RED}No proxies found in {file_path}")
        return proxies
...
def get_random_proxy(proxies):
    ...
    print(f"{get_wib_timestamp()} | {Fore.RED}No proxies available!")
    return {}
...
def check_proxy(proxy):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"{get_wib_timestamp()} | {Fore.YELLOW}[LIVE PROXY] {ip}")
            return True, ip
    except Exception:
        print(f"{get_wib_timestamp()} | {Fore.RED}[-] Proxy dead: {proxy['http']}")
    return False, None
...
def login_account(email, pw, proxies, max_retries=5):
    data = {
        "blockchain": "testnet",
        "email": email,
        "pw": pw,
    ...
    while retries < max_retries:
        ...
        if not check_proxy(proxy)[0]:
            print(f"{get_wib_timestamp()} | {Fore.YELLOW}[BAD PROXY] Proxy tidak bisa digunakan, ganti proxy...")
            retries += 1
            continue
        try:
            proxy_dict = {"http": proxy["http"], "https": proxy["https"]}
            response = requests.post(SIGNIN_URL, headers=HEADERS, data=data, proxies=proxy_dict, timeout=20)
            ...
            retries += 1
            wait_time = 2 ** retries
            print(f"{get_wib_timestamp()} | {Fore.RED}[ERROR] {email} -> Proxy error: {e}, retry {retries}/{max_retries} in {wait_time}s")
            time.sleep(wait_time)
    print(f"{get_wib_timestamp()} | {Fore.RED}[ERROR] {email} -> Max retries exceeded.")
    return False

def fetch_daily_status(data, proxy, retries=7, backoff_factor=1.5):
    payload = {
        "blockchain": "testnet",
    ...
    while attempt < retries:
        try:
            response = requests.post(CHECKIN_URL, headers=HEADERS, data=payload, proxies=proxy, timeout=15)
            if response.status_code == 200:
...
def get_quiz_data(data, proxy, retries=7, backoff_factor=1.5):
    payload = {
        "blockchain": "testnet",
        "address": data['address'],
        "lang": "id",
    ...
    while attempt < retries:
        try:
            response = requests.post(QUIZ_QUESTION_URL, data=payload, headers=HEADERS, proxies=proxy, timeout=15)
            if response.status_code == 200:
...
def submit_quiz_answer(quiz_idx, answer_idx, data, proxy, retries=7, backoff_factor=1.5):
    payload = {
        "blockchain": "testnet",
        "address": data['address'],
        "quiz_idx": quiz_idx,
    ...
    while attempt < retries:
        try:
            response = requests.post(QUIZ_ANSWER_URL, data=payload, headers=HEADERS, proxies=proxy, timeout=15)
            if response.status_code == 200:
                data_res = response.json()
...
def check_events(data, proxy, retries=7, backoff_factor=2):
    payload = f"blockchain=testnet&lang=id&device=app&is_mobile=Y&session_code={data['session_code']}"
    attempt = 0
    while attempt < retries:
        try:
            response = requests.post(EVENTS_URL, headers=HEADERS, data=payload, proxies=proxy, timeout=15)
            if response.status_code in [200, 201]:
...
def process_event(data, event_id, proxy, retries=7, backoff_factor=2):
    ...
    while attempt < retries:
        try:
            response = requests.post(url, headers=HEADERS, data=payload, proxies=proxy, timeout=15)
            if response.status_code in [200, 201]:
                result = response.json()
...
def process_account(data, quiz_idx, answer_idx, proxies, skip_completed=True):
    print(f"{get_wib_timestamp()} | {Fore.BLUE}Processing account: {data['email']}")
    
    retry_proxy = 0
    proxy = {}
    ip = None
    while retry_proxy < 10:
        proxy = get_random_proxy(proxies)
        is_live, ip = check_proxy(proxy)
        if is_live:
            break
        retry_proxy += 1
        print(f"{get_wib_timestamp()} | {Fore.YELLOW}Proxy {proxy['http']} failed. Retrying... ({retry_proxy}/10)")
    if not is_live:
        print(f"{get_wib_timestamp()} | {Fore.RED}No live proxy after 10 attempts. Skipping account.")
        return

    data_events = check_events(data, proxy)
    ...
    for event in all_events:
        ...
        process_event(data, event_id, proxy)
...
def get_session_codes():
    ...
    if not proxies:
        print(f"{get_wib_timestamp()} | {Fore.RED}No proxies available. Exiting.")
        return
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = [executor.submit(login_account, email, pw, proxies) for email, pw in akun_list]
...
def process_main():
    ...
    while retry_proxy < 10:
        ...
        print(f"{get_wib_timestamp()} | {Fore.YELLOW}Proxy {proxy['http']} failed. Retrying... ({retry_proxy}/10)")
    if not is_live:
        print(f"{get_wib_timestamp()} | {Fore.RED}No live proxy after 10 attempts. Skipping quiz data retrieval.")
        return

    quiz_idx, quiz_title, quiz_q = get_quiz_data(first_data, proxy)
    answer_idx = None
    if quiz_idx:
        answer_idx = input(f"{get_wib_timestamp()} | {Fore.MAGENTA}Masukkan id Quiz (e.g., 388): {Style.RESET_ALL}").strip()

    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = [executor.submit(process_account, data, quiz_idx, answer_idx, proxies) for data in data_list]
        for future in as_completed(futures):
            pass
